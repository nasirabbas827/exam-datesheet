...
// Function to calculate exam schedule with minimum days and correct non-overlapping criteria
function calculateExamSchedule($slots_per_day, $non_overlapping_courses, $superintendents, $halls) {
    ...
    // Loop through each course and assign to days
    foreach ($non_overlapping_courses as $course_id => $courses) {
        // Find a suitable superintendent and hall
        $assigned_superintendent = "No superintendent assigned";
        $assigned_hall = "No suitable hall found";

        // Check if there are superintendents available for this course's faculty
        $available_superintendents = [];
        foreach ($superintendents as $superintendent_id => $superintendent) {
            // Check if the superintendent teaches this course
            if (in_array($courses[0], $superintendent['courses'])) { // Assuming courses[0] is the course code
                continue; // Skip assigning this superintendent
            }
        ...
        // Place courses, superintendent, and hall in schedule for the current day
        $schedule[$day_count]['courses'] = $courses;
        $schedule[$day_count]['superintendent'] = $assigned_superintendent;
        $schedule[$day_count]['hall_name'] = $assigned_hall;

        // Update day courses with the current courses
        $day_courses[$day_count] = $courses;

        // Check if current courses overlap with previously scheduled courses in the same day
        foreach ($day_courses as $index => $day_courses_check) {
            // Convert both arrays to sets for easier comparison
            $current_courses_set = array_flip($courses);
            $day_courses_check_set = array_flip($day_courses_check);

            // Check for intersection
            $overlap = array_intersect_key($current_courses_set, $day_courses_check_set);
            if (!empty($overlap)) {
                // Courses overlap, move to the next day
                $day_count++;
                if ($day_count % $slots_per_day == 0) {
                    $day_count = 0;
                }
                // Restart the loop to reassign superintendent and hall
                continue 2;
            }
        }

        // If no overlap, increment day count
        $day_count++;
        if ($day_count % $slots_per_day == 0) {
            $day_count = 0;
        }
